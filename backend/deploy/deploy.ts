...
const main: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
    ...
    if (chainId == 31337) {
      ...
      const FUND_AMOUNT = ethers.parseEther("1")
        ...
        const WEI_PER_LINK = "5000000000000000" // 0.005 LINK per wei
        // create VRFV2 Subscription
        await deploy("VRFCoordinatorV2_5Mock", {
          from: deployer,
          args: [BASE_FEE, GAS_PRICE_LINK, WEI_PER_LINK],
          log: true,
        });
        const { address } = await deployments.get("VRFCoordinatorV2_5Mock");
        ...
        const subscriptionEvent = txReceipt.logs.find(
          (log) => log.fragment?.name === "SubscriptionCreated"
        );
        subscriptionId = subscriptionEvent?.args.subId;
