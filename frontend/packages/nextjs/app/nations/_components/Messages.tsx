...
const MessagesComponent = () => {
  ...
  return (
    <div className="font-special bg-aged-paper text-base-content p-6 rounded-lg shadow-center w-full max-w-2xl mx-auto border border-neutral">
      <h2 className="text-2xl font-bold text-primary-content text-center mb-4">📜 Nation Messages for {nationName}</h2>
        ...
        {receivedMessages.length > 0 ? (
          <div className="space-y-4">
            {receivedMessages.map(msg => (
              <div
                key={msg.id}
                className="p-4 bg-base-100 border border-neutral text-base-content shadow-md rounded-md break-words"
              >
                <p>
                  <strong>From:</strong> {msg.sender}{" "}
                  <span className="text-primary">({nationNames[msg.sender] || "Loading..."})</span>
                </p>
                <p>
            ...
            {sentMessages.map(msg => (
              ...
              >
                <p>
                  ...
                  <span className="text-primary">({nationNames[msg.receiver] || "Loading..."})</span>
                </p>
                <p>
                  <strong>Message:</strong> {msg.message}
                </p>
      ...
      {statusMessage && (
        <p className="mt-4 text-center text-sm text-secondary-content bg-secondary p-2 rounded-lg">{statusMessage}</p>
      )}
    </div>
  );
};
