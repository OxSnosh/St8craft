...
import { getAllianceMembers, getJoinRequests, getNationAlliance, getNationAllianceAndPlatoon } from "~~/utils/alliance";
import {
  addAdmin,
  approveNationJoin,
  assignNationToPlatoon,
  isAdmin as checkIsAdmin,
  createAlliance,
  removeAdmin,
  removeNationFromAlliance,
...
const AllianceManagement = () => {
  ...
  useEffect(() => {
    const fetchNations = async () => {
      ...
      try {
        ...
        const getNationName = async (nationId: string) =>
  ...
  useEffect(() => {
    ...
    const fetchNationAlliance = async () => {
      try {
        const [alliance, platoon, allianceName] = await getNationAllianceAndPlatoon(
          selectedNation,
          publicClient,
  ...
  useEffect(() => {
    console.log(
      "Updated State -> Alliance:",
      currentNationAlliance,
      "Platoon:",
      currentNationPlatoon,
      "Alliance Name:",
      currentNationAllianceName,
    );
  ...
  const handleAssignPlatoon = async () => {
    ...
    if (currentNationAlliance && assignPlatoonNationId && platoonId && selectedNation) {
      await assignNationToPlatoon(
        currentNationAlliance,
        assignPlatoonNationId,
        platoonId,
        selectedNation,
        publicClient,
        CountryParametersContract,
  ...
  const handleRemoveNation = async (nationId: string) => {
    ...
    await removeNationFromAlliance(
      currentNationAlliance,
      nationId,
      selectedNation,
      publicClient,
        ...
        >
          ...
          {nations.map(nation => (
            <option key={nation.id} value={nation.id}>
              {nation.id.toString()}: {nation.name}
            </option>
          ))}
        </select>
      </div>
        ...
        <>
          ...
          <div className="p-4 bg-base-200 rounded-lg shadow-md mt-4">
            ...
            <h4 className="text-lg font-semibold text-primary">
              Once you create an allicance your nation will not be able to leave
            </h4>
            <input
              type="text"
            ...
            />
            <button onClick={handleCreateAlliance} className="btn btn-primary w-full mt-3">
              Create Alliance
            </button>
          </div>
          ...
          <div className="p-4 bg-base-200 rounded-lg shadow-md mt-4">
            ...
            <input
              ...
              className="input input-bordered w-full bg-base-100 text-base-content mt-2"
            />
            <button onClick={handleRequestToJoin} className="btn btn-accent w-full mt-3">
              Request to Join
            </button>
          </div>
            ...
            <>
              ...
              <div className="p-4 bg-base-200 rounded-lg shadow-md mt-4">
                ...
                {currentNationAlliance && currentNationAlliance !== "0" ? (
                  <>
                    <p className="text-base text-secondary-content">Alliance ID: {currentNationAlliance.toString()}</p>
                    <p className="text-base text-secondary-content">Alliance Name: {currentNationAllianceName}</p>
                    <p className="text-sm font-medium mt-2">
                      Platoon: {currentNationPlatoon?.toString() || "Not Assigned"}
                    </p>
                  </>
                ) : (
                ...
                <>
                  ...
                  <div className="p-4 bg-base-200 rounded-lg shadow-md mt-4">
                    ...
                    <input
                      ...
                      className="input input-bordered w-full bg-base-100 text-base-content mt-2"
                    />
                    <button onClick={handleAssignPlatoon} className="btn btn-primary w-full mt-3">
                      Assign Platoon
                    </button>
                  </div>
                  ...
                  <div className="p-4 bg-base-200 rounded-lg shadow-md mt-4">
                    ...
                    <button onClick={handleApproveJoin} className="btn btn-success w-full mt-3">
                  ...
                  <div className="p-4 bg-base-200 rounded-lg shadow-md mt-4">
                    ...
                    <div className="mt-3 flex gap-2">
                      <button onClick={handleAddAdmin} className="btn btn-accent flex-1">
                        Add Admin
                      </button>
                      <button onClick={handleRemoveAdmin} className="btn btn-error flex-1">
                        Remove Admin
                      </button>
                    </div>
                  </div>
                  ...
                  <ul className="list-disc pl-5">
                    {allianceMembers.map(member => (
                      <li key={member} className="flex justify-between items-center mt-2">
                        <span>
                          {member.toString()}: {nationNames[member] || "Loading..."}
                        </span>
                        {isAdmin && (
                          <button onClick={() => handleRemoveNation(member)} className="btn btn-error btn-sm">
                            Remove
                          </button>
                        )}
                      </li>
